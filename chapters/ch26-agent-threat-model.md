...
**Unauthenticated Inspector Access** (CVE-2025-49596, CVSS 9.4) allowed arbitrary command execution via MCP Inspector instances that lacked authentication. This class of vulnerability is particularly dangerous in Windows 365 deployments where the MCP Inspector may be left accessible on loopback without realizing it is reachable from other processes.

The NSA released a formal advisory on MCP security in 2025, noting that the protocol specification itself states it "cannot enforce these security principles at the protocol level" — meaning the responsibility for MCP security falls entirely on the deployment layer. Chapter 28 addresses MCP server allow-listing and sandboxing controls for Claude Code.
