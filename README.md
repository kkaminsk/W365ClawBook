...
1. **Replicate the ACG image version** to the DR region (configured in the Terraform `replica_count` and `target_regions` settings; see Chapter 20).
...
graph TB
    ...
    subgraph "Build Pipeline"
        ...
        AIB[Azure VM Image Builder<br/>Ephemeral Build VM<br/>Standard_D2s_v5]
    end
...
**Layer 2: Build Pipeline.** A manual `terraform apply` deploys the infrastructure and triggers the AIB build. The build VM (Standard_D2s_v5 by default) downloads installers, runs four phases of PowerShell customization, applies Windows Updates, and runs Sysprep. The result is a generalized VHD published to the Azure Compute Gallery. Build time: 75–120 minutes.

> **💡 Note:** The default build VM is `Standard_D2s_v5` (2 vCPU, 8 GB RAM) for cost efficiency. If build speed is a priority, upgrade to `Standard_D4s_v5` (4 vCPU, 16 GB) in `terraform.tfvars`, which typically reduces build time by 15–25 minutes, particularly during Windows Update and npm install phases.
...
For organizations with Azure Network Connections in multiple regions (e.g., East US 2 for primary and Central US for DR), configure multi-region replication in the Terraform module by adding target regions to the image version:

```hcl
variable "target_regions" {
  description = "Regions to replicate image versions to"
  type = list(object({
    name                   = string
